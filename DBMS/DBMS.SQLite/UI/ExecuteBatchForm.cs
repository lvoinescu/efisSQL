...
namespace DBMS.SQLite
{
	public partial class ExecuteBatchForm : Form
	{
		...
		public ExecuteBatchForm(string fileName, SQLiteConnection connection )
		{
			...
			try{
				...
				{
					SQLiteCommand cmdb = new SQLiteCommand("use `" + browser.CurrentDatabase +"`;", connection);
		...
		private void parser_QueriesRead(object senderParser, QueriesReadEventArgs arg) {
			...
			if (!ReadOnly)
			{
				...
				{
					cmd = new SQLiteCommand(arg.Queries[i], connection);
					try
					{
						cmd.ExecuteNonQuery();
					}
		...
		private void dg_CellContentClick(object sender, DataGridViewCellEventArgs e)
		{
			...
			if (e.ColumnIndex == 2)
			{
				...
				if (e.ColumnIndex == 3)
			{
				using (SQLiteCommand cmd = new SQLiteCommand((string)dgList[e.RowIndex].Data.ToString(), connection))
				{
					try
					{
